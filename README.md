...
Set `rep.weights.positive` and `rep.weights.negative` in `config.yml` (defaults `1` and `-2`), then run `/rep admin reload`.
For equal positive/negative weight, use `positive: 1` and `negative: -1`.
Use signed whole numbers: positive must be greater than zero and negative less than zero. Invalid values fall back to the defaults.
New votes and changes between positive and negative use the configured weight. Existing votes keep their saved weight on reload, restart, reason/category edits, and restoration; removing a vote reverses its saved value. This does not retroactively recalculate scores.
